...
class CampusControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create campu" do
    assert_difference('Campu.count') do
      post campus_url, params: { campu: { location: @campu.location, map: @campu.map, name: @campu.name } }
    end
  ...
  test "should update campu" do
    patch campu_url(@campu), params: { campu: { location: @campu.location, map: @campu.map, name: @campu.name } }
